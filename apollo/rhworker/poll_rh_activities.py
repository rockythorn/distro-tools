...
async def process_csaf_files() -> dict:
    ...
    look_back_days = 20  # Process advisories created or modified in the last X days
    ...
    async with aiohttp.ClientSession() as session:
        ...
        logger.info(f"Found {len(filtered_advisory_ids)} advisories to process from the last {look_back_days} days")
